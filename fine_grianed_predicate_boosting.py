...
from models.MLM.model import VisualBertPromptModel

# fine = ['riding', 'carrying', 'eating', 'walking on', 'playing', 'covering', 'laying on', 'attached to', 'watching', 'sitting on', 'between', 'belonging to', 'painted on', 'against', 'looking at', 'hanging from', 'parked on', 'in front of', 'made of', 'covered in', 'mounted on', 'says', 'part of', 'across', 'flying in', 'using', 'on back of', 'lying on', 'growing on', 'walking in', 'standing on']
target_words = [line.strip('\n').strip('\r') for line in open('/home/qifan/datasets/vg/predicate_list.txt')]
# mapping_dict = json.load(open('fine_predicate_mapping_dict.json'))
fine_words_dataset = fineTuningDataset('image_caption_triplet.json',"/home/qifan/datasets/coco/train2014/",'train')
...
def predict_rel_prompt(model, img_path, subject, object, is_open_world, device='cuda'):
    predicate = '[MASK]'
    batch_text = []
    batch_img = []
    ...
    if torch.max(torch.softmax(predictions[0], dim=0)).item() < 0.25:
        return expand_results
    word_candidates = torch.argsort(predictions[0], descending=True)[:3].tolist()

    mapping_dict = json.load(open('utils_data/mapping/predicate_mapping_dict.json'))

    # increase diversity of expanded words (without mapping) 
    if is_open_world:
        if random.random() > 0.5:
            expand_results.append((batch_text[0][0], model.word_table[word_candidates[0]], batch_text[0][2]))  
        else:
            i = random.randint(1, len(word_candidates)-1)
            expand_results.append((batch_text[0][0], model.word_table[word_candidates[i]], batch_text[0][2]))  
    else:
        if random.random() > 0.5:
            mapping_words = mapping_dict[model.word_table[word_candidates[0]]]
        else:
            i = random.randint(1, len(word_candidates)-1)
            mapping_words = mapping_dict[model.word_table[word_candidates[i]]]
...
def expand_relationships(rel_data, obj_data, img_data, split, encoded_label, idx_to_label, im_to_first_obj, im_to_last_obj, encoded_boxes, expand_predicate_to_idx, is_open_world):
    prompt_candidates = []
    ...
    # expand relationships by all possible relation and project to 50 classes(base+novel) and we can weight for base and novel (Two separate enhancements)
    model_own.load_state_dict(torch.load('pre_trained_visually_prompted_model/model_final_vis.pkl'))
    # len(obj_data) == len(rel_data) == 108730
    n = 0 
    max_rel_id = 4836654
    new_predicate_dict = {}
    test_n = 0 # only 10 train images for test
    expand_dataset = dict()
    final_triplets = np.load("final_triplets.npy").tolist()
    rst = []
    for i, rel_info in tqdm(enumerate(rel_data)):
        if split[i] == 0:
            ...
            for inter_object in inter_objects:
                ...
                if subject['object_id'] in id_to_idx and object['object_id'] in id_to_idx:
                    if (id_to_idx[subject['object_id']], id_to_idx[object['object_id']]) not in original_objects:
                        # original object id which need to choose original predicates or novel predicates
                        # add more novel predicates with these NA relations
                        # 'encoder_label' maps the object'idxs to object_list_idx
                        ...
                        # if id_to_idx[subject['object_id']] != id_to_idx[object['object_id']]:
                        if subject_label != object_label:
                            # expand_data_information: ['width', 'height', 'img_path', 'boxes', 'labels', 'relations'], save external relationship to pk
                            predicate_results = predict_rel_prompt(model_own, img_path, subject_label, object_label, is_open_world)
                            # select most similar predicate in dataset for augmentation
            ...
            new_rel_dict['relations'] = np.array(new_rel_dict['relations'])
            rst.append(new_rel_dict)
            n += len(new_rels)
            # expand original data with base (open-world setting and standard setting)
            rel_info['relationships'].extend(new_rels)
            # only use the expanded dataset without original data (unsupervised setting)
            # rel_info['relationships'] = new_rels
    pickle.dump(rst, open('extra_open-world_expand.pk', "wb"))
    # json.dump(expand_dataset, open('expand_dataset_label.json','w'))
...
def encode_splits_base(obj_data, rel_data, data_split, opt=None):
    if opt is not None:
    ...
    return split
def encode_splits_open(obj_data, rel_data, base_predicate, data_split, opt=None):
    ...
    for i, info in enumerate(rel_data):
        ...
        if opt is None: # use encode from input file
            ...
            if i >= test_begin_idx: splitix = 2
        split[i] = splitix
    # filter traom set and valid set(keep both coarse-grained and fine-grained)
    # for open-world, we evaluate both base classes and novel classes
    remain_num = 0
    total_num = 0 
...
def main(args):
    ...
    # build vocabulary for original dataset
    ...
    print('object_classess: ',len(idx_to_label))
    print('predicate_classess: ',len(idx_to_predicate))
    ...
    # open-wrold setting
    base_predicates = predicate_sorted_name[:25]
    novel_predicates = predicate_sorted_name[25:]
    ...
    for p in words:
        # add the infrequent predicate into dic
    ...
    expand_predicate_tokens, expand_predicate_token_counter = extract_predicate_token(rel_data,len(expand_pred_list),expand_pred_list)
    ...
    for p in base_predicates:
        ...
        # when in base setting and ow-setting, target_predicate both need for all 50 predicates
        novel_predicate_tokens.add(p)
    ...
    if args.open_world:
        # special split for open-world setting of SGG
        split = encode_splits_open(obj_data, rel_data, base_predicate_tokens, data_split, opt)
    else:
        # original split for base setting of SGG
        split = encode_splits_base(obj_data, rel_data, data_split, opt)
    ...
    expand_number, new_predicate_dict = expand_relationships(rel_data, obj_data, img_data, split, encoded_label, idx_to_label, im_to_first_obj, im_to_last_obj, encoded_boxes, expand_predicate_to_idx, args.open_world)
    print('expand relationships: ', expand_number)
    print('new_predicate_dict: ', new_predicate_dict)
    # update expand_predicate_token_counter by new predicates from CaCao
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json_file_base', default='/home/qifan/datasets/vg/VG-SGG-base-dicts.json') # without boost from CaCao
    parser.add_argument('--h5_file_base', default='/home/qifan/datasets/vg/VG-SGG-base.h5') # without boost from CaCao
    parser.add_argument('--load_frac', default=1, type=float)
    parser.add_argument('--use_input_split', default=True, type=bool)
    parser.add_argument('--input_split_file', default="/home/qifan/datasets/vg/50/VG-SGG-with-attri.h5")
    parser.add_argument('--train_frac', default=0.7, type=float)
    parser.add_argument('--val_frac', default=0.8, type=float)
    parser.add_argument('--shuffle', default=False, type=bool)
    parser.add_argument('--open_world', default=False, type=bool)
