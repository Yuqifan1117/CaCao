...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        self, input_ids=None, visual_prompts=None, token_type_ids=None, position_ids=None, inputs_embeds=None, past_key_values_length=0
    ):
        ...
        if visual_prompts is not None:
            embeddings = torch.cat((visual_prompts,embeddings),dim=1)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config, prompt_ids = None):
        super().__init__(config)
        ...
        self.bert = BertModel(config, add_pooling_layer=False, prompt_ids=prompt_ids)
        self.cls = BertOnlyMLMHead(config)
    ...
    def forward(
        ...
        if labels is None:
            ...
            # label should be padding with -100 for other positions
            mask_pos = torch.where(labels!=-100)[1]
            loss_div = div_regularization(prediction_scores.view(-1, self.config.vocab_size)[mask_pos])
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))# + 0.5 * loss_div
